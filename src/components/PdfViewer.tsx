...
import { Search, X, ChevronLeft, ChevronRight, Highlighter } from "lucide-react";
...
const PdfViewer = ({ content, title, highlights = [], onAddHighlight, onRemoveHighlight }: PdfViewerProps) => {
  ...
  // Highlight matching text in page content
  const getHighlightedContent = (pageContent: string, pageIdx: number) => {
    const pageHighlights = highlights.filter(h => h.pageIndex === pageIdx);
    if (pageHighlights.length === 0 && !searchQuery) return pageContent;
    return pageContent;
  };
      ...
      <div className="flex-1 overflow-y-auto p-4" onMouseUp={handleTextSelect}>
        <div className="max-w-[600px] mx-auto">
          ...
          >
            ...
            <div className="prose prose-sm max-w-none text-gray-900 dark:text-foreground 
      ...
      <div className="flex items-center justify-center gap-4 py-3 bg-card border-t border-border shrink-0">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            value={currentPage + 1}
            onChange={(e) => {
              const p = Math.max(0, Math.min(totalPages - 1, Number(e.target.value) - 1));
              setCurrentPage(p);
            }}
